...
　<br>
GOTO:指定された行数にジャンプします。指定された行数が、コードの範囲内に収まっている必要があります。(いずれ行数指定ではなくラベルなどを使った方式へと変更し、最終的には削除します)<br>
...
EXPORT:引数の内容を配列に追加。そして、Engine.getExportValues()でその内容を取り出すことが出来ます。これによって、Java間で情報のやり取りがより簡単にできるようになります。<br>
 <br>
 ## 新しいコマンド追加チュートリアル
<br>
 工事中！！
